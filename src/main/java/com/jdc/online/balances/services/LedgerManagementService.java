...
public class LedgerManagementService {
	...
	public void save(LedgerForm ledgerForm) {
		var entity = Optional.ofNullable(ledgerForm.getId()).flatMap(id -> ledgerRepo.findById(id))
				.orElse(new Ledger());
	...
	public PageResult<LedgerListItem> search(String username, LedgerSearch search, int page, int size) {

		return ledgerRepo.search(queryFunc(search, username), countFunc(search, username), page, size);
	}

	private Function<CriteriaBuilder, CriteriaQuery<LedgerListItem>> queryFunc(LedgerSearch search, String username) {
	...
	private Function<CriteriaBuilder, CriteriaQuery<Long>> countFunc(LedgerSearch search, String username) {
		return cb -> {
	...
	private Function<CriteriaBuilder, CriteriaQuery<LedgerSelectItem>> queryFunc(String username, BalanceType type) {
		return cb -> {
			...
			cq.multiselect(root.get(Ledger_.id), root.get(Ledger_.name));
			cq.where(cb.equal(root.get(Ledger_.member).get(Member_.account).get(Account_.username), username),
