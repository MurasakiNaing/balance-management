...
import com.jdc.online.balances.model.entity.embeddable.LedgerEntryPk_;
...
public class MemberBalanceService {
	...
	private Function<CriteriaBuilder, CriteriaQuery<BalanceListItem>> queryFunc(String username, BalanceSearch search) {
		...
		return cb -> {
			...
			cq.orderBy(cb.asc(root.get(LedgerEntry_.id).get(LedgerEntryPk_.issueDate)), cb.asc(root.get(LedgerEntry_.id).get(LedgerEntryPk_.seqNumber)));
